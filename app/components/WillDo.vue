<template>
	<div class="card-content">
		...
		<submission-form v-bind:visible="forms.submission.visible" v-bind:from="'editform'" v-bind:formOpers="forms.submission.formOpers"
			...
			batchOper(approve) { //待办的批量处理
				if (this.listChecks.length === 0) {
					...
					if (stageSet.size > 1) {
						...
						} else if (['reject', 'surveyPrepare', 'survey', 'argue', 'auditFirst', 'auditSecond'].indexOf(stage) !== -1) {
							Notification.error({
								title: '操作失败!',
								message: '当前阶段不可批量操作！',
								duration: 3000
